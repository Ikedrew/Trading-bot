...
"position_excursion": ResearchDisposition(
    ...
    "market_context": ResearchDisposition(
        ...
        phase2=Phase2Disposition.A_RESEARCH_INPUT,
        status=ResearchDispositionStatus.DIRECTLY_CONSUMED,
        reason="Canonical factual market-state environment (regime/phase/H4/H1/M15/M5 summaries) feeding the MARKET universe.",
    ...
    "opportunities": ResearchDisposition(
        ...
        status=ResearchDispositionStatus.DIRECTLY_CONSUMED,
        reason=(
            "Canonical opportunity lifecycle records (what the system detected, "
            "state, rejection_stage, pattern, scores) — system-created, "
            "decision-derived evidence forming the DECISION universe base "
            "population. Not raw market fact."
        ),
        research_purpose=(
            "Opportunity quality / rejection-stage analysis; missed-opportunity "
            "counterfactuals using decision-derived evidence."
        ),
        ...
        temporal_availability=TemporalAvailability.BEFORE_DECISION,
        lineage_guard_notes=(
            "Opportunity evidence is articulated BEFORE the terminal decision and "
            "is pre-decision-safe, but it is still generated by the decision "
            "system rather than raw market evidence."
        ),
    ),
