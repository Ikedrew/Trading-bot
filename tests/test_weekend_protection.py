...
class TestFlattenDisabled:
    ...
    def test_flatten_closes_positions(self, default_config):
        """Flatten enabled ? positions closed via execution layer."""
        mock_pos = MagicMock()
        ...
        mock_tm = MagicMock()
        mock_tm.positions_open.return_value = [mock_pos]

        mock_exec = MagicMock()
        mock_exec.close_position.return_value = MagicMock(ok=True)

        closed = flatten_all_positions(
            trade_managers=[mock_tm],
            execution=mock_exec,
        )
